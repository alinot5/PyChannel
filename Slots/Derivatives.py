...
class Derivatives: # This will be the GPU testing enviroment
    ...
    def __init__(self,N=[35,32,32,3,1],L=[2,.875*2*math.pi,.6*2*math.pi],Re=400):
        ...
        self.k=[cp.repeat(k[0][:,:,:,cp.newaxis],N[-2],axis=-1),cp.repeat(k[1][:,:,:,cp.newaxis],N[-2],axis=-1)]
        ...
        # ufftwb = pyfftw.empty_aligned((self.N[0],self.N[1], self.N[2],self.N[3],1), dtype='float64')
        # ufftwf= pyfftw.empty_aligned((self.N[0],self.N[1], int(self.N[2]/2)+1,self.N[3],1), dtype='complex128')
        self.ufft_temp=cp.zeros((self.N[0],self.N[1], self.N[2],self.N[3],1),dtype='complex128')
        # self.ufft_object = pyfftw.FFTW(ufftwb, ufftwf,axes=(1,2),flags=('FFTW_MEASURE',), direction='FFTW_FORWARD',threads=threads)
        # # pressure
        # Pfftwb = pyfftw.empty_aligned((self.N[0],self.N[1], self.N[2],1,1), dtype='float64')
        # Pfftwf= pyfftw.empty_aligned((self.N[0],self.N[1], int(self.N[2]/2)+1,1,1), dtype='complex128')
        self.Pfft_temp=cp.zeros((self.N[0],self.N[1], self.N[2],1,1),dtype='complex128')
        # self.Pfft_object = pyfftw.FFTW(Pfftwb, Pfftwf,axes=(1,2),flags=('FFTW_MEASURE',), direction='FFTW_FORWARD',threads=threads)
        # Store IFFT things
        # velocity
        # uifftwb = pyfftw.empty_aligned((self.N[0],self.N[1], self.N[2],self.N[3],1), dtype='float64')
    ...
    def convert(self,u,ufft):
        ...
        Nzp=int(self.N[2]/2)
        ufft[:,:,:Nzp+1,:]=u
        ufft[:,0,self.N[2]-Nzp+1:,:]=cp.conjugate(cp.flip(u[:,0,1:Nzp,:],axis=1))
        ufft[:,Nxp,self.N[2]-Nzp+1:,:]=cp.conjugate(cp.flip(u[:,Nxp,1:Nzp,:],axis=1))
        ufft[:,self.N[1]-Nxp+1:,self.N[2]-Nzp+1:,:]=cp.conjugate(cp.flip(cp.flip(u[:,1:Nxp,1:Nzp,:],axis=1),axis=2))
        ufft[:,1:Nxp,self.N[2]-Nzp+1:,:]=cp.conjugate(cp.flip(cp.flip(u[:,self.N[1]-Nxp+1:,1:Nzp,:],axis=1),axis=2))
        return ufft

    # These are the functions used for moving between real, fourier, and chebyshev space.
    def fft(self,u):

        ufft = cp.fft.fftn(u, axes=(1,2))
        ufft = ufft / (self.N[1] * self.N[2])
        return ufft
        # if u.shape[3]==3: #velocity
        #     ufft=self.ufft_object(u)
        #     self.ufft_temp=self.convert(ufft,self.ufft_temp)
    ...
    def ifft(self,u):
        uifft = cp.fft.ifftn(u,axes=(1,2))
        uifft = uifft * self.N[1] * self.N[2]
        return uifft
        # if u.shape[3]==3: #velocity
        #     uifft=self.uifft_object(u[:,:,:int(self.N[2]/2)+1,:])
        #     uifft=self.N[1]*self.N[2]*uifft
    ...
    def cheb(self,u,threads=-1):
        ...
        U = cp.fft.fft(u_ext, axis = 0)

        U = U[:self.N[0]]

        k = cp.arange(self.N[0])[:, None, None, None, None]
        U = (U + u[0]) * ((-1)**k) + u[-1]
        U = U / (self.N[0] - 1)
    ...
    def dx(self,a,inpy=0,outy=0,inpxz=0,outxz=0):
        ...
        # Calculate the derivative (K is for if it isn't u,v, and w)
        [_,_,_,K,_]=a.shape
        dadx=1j*self.k[0][:,:,:,0:K,cp.newaxis]*(2*math.pi/(self.L[1]))*a
        # Return derivative
        if outxz==1:
            dadx=self.ifft(dadx)
    ...
    def dz(self,a,inpy=0,outy=0,inpxz=0,outxz=0):
        ...
        # Calculate the derivative (K is for if it isn't u,v, and w)
        [_,_,_,K,_]=a.shape
        dadz=1j*self.k[1][:,:,:,0:K,cp.newaxis]*(2*math.pi/(self.L[2]))*a
        # Return derivative
        if outxz==1:
            dadz=self.ifft(dadz)
    ...
    #inp[0,1] -> [cheb,real]
    def dy(self,a,inpy=0,outy=0,inpxz=0,outxz=0):
        # Ensure a is complex type for spectral calc
        a = a.astype(cp.complex128)

        # Convert into Chebyshev if input is in physical space
        if inpy == 1:
            a = self.cheb(a)

        dady = self.cheb_derivative(a)
    ...
    def dx2(self,a,inpy=0,outy=0,inpxz=0,outxz=0):
        ...
        # Calculate the derivative (K is for if it isn't u,v, and w)
        [_,_,_,K,_]=a.shape
        dadx=-(self.k[0][:,:,:,0:K,cp.newaxis]*(2*math.pi/(self.L[1])))**2*a
        # Return derivative
        if outxz==1:
            dadx=self.ifft(dadx)
        # Return cheb or real
    ...
    def dz2(self,a,inpy=0,outy=0,inpxz=0,outxz=0):
        ...
        # Calculate the derivative (K is for if it isn't u,v, and w)
        [_,_,_,K,_]=a.shape
        dadz=-(self.k[1][:,:,:,0:K,cp.newaxis]*(2*math.pi/(self.L[2])))**2*a
        # Return derivative
        if outxz==1:
            dadz=self.ifft(dadz)
        # Return cheb or real
    ...
    def dy2(self,a,inpy=0,outy=0,inpxz=0,outxz=0):
        ...
        if inpy==1:
            a=self.cheb(a)
        # Get the derivative
        dady=cp.zeros(a.shape,dtype=cp.complex128)
        a_cpu = cp.asnumpy(a)

        dady_cpu = np.polynomial.chebyshev.chebder(np.polynomial.chebyshev.chebder(a_cpu))
        dady[:-2,:,:,:,:]= cp.asarray(dady_cpu) #HERE
        # Return derivative
        if outy==1:
            dady=self.icheb(dady)
        # Return fft or real
    ...
    def gradu(self,a,inpy=0,outy=0,inpxz=0,outxz=0):
        ...
        dx=self.dx(a,inpy,outy,inpxz,outxz); dy=self.dy(a,inpy,outy,inpxz,outxz); dz=self.dz(a,inpy,outy,inpxz,outxz)    
        # Concatenate for gradu
        gradu=cp.concatenate((dx[:,:,:,cp.newaxis,:,:],dy[:,:,:,cp.newaxis,:,:],dz[:,:,:,cp.newaxis,:,:]),axis=-3)
        return gradu
    ...
    def uu(self,a,inpy=0,outy=0,inpxz=0,outxz=0):
        ...
        uv=uv[:,:,:,:,cp.newaxis,:]
        uw=uw[:,:,:,:,cp.newaxis,:]
        uu_out=cp.concatenate((uu,uv,uw),axis=-2)    
        return uu_out
    ...
    def H(self,a,inpy=0,outy=0,inpxz=0,outxz=0):
        ...
        if inpxz==0:
            a=self.ifft(a)
        # Skew-symmetric
        ugradu=cp.einsum('ijkln,ijklmn->ijkmn',a, gradu)
        if outy==0:
            ugradu=self.cheb(ugradu)
        if outxz==0:
            ugradu=self.fft(ugradu)
