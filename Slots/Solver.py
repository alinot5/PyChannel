...
class Solver(Derivatives):
    ...
    def __init__(self,N=[35,32,32,3,1],L=[2,.875*2*math.pi,.6*2*math.pi],store=True,Re=400):
        ...
        y=cp.arange(N[0])
        self.y=-cp.cos(y*math.pi/(N[0]-1))
        
        self.base=cp.zeros((N[0],N[1],N[2],N[3]))
        self.base[:,:,:,0]=cp.flip(cp.repeat(cp.repeat(self.y[:,cp.newaxis,cp.newaxis],N[1],axis=1),N[2],axis=2),0)

        # Get the wavenumbers
        k=cp.meshgrid(cp.fft.fftfreq(N[1], d=1/N[1]),cp.fft.fftfreq(N[2], d=1/N[2]))
        k=[k[0].transpose(),k[1].transpose()]
        k=[cp.repeat(k[0][cp.newaxis,:,:],N[0],axis=0),cp.repeat(k[1][cp.newaxis,:,:],N[0],axis=0)]
        self.k=[cp.repeat(k[0][:,:,:,cp.newaxis],N[-2],axis=-1).astype('complex128'),cp.repeat(k[1][:,:,:,cp.newaxis],N[-2],axis=-1).astype('complex128')]
        ...
        self.Ps=cp.zeros((self.N[0]-2))
        self.Qs=cp.zeros((self.N[0]-2))
        self.Rs=cp.zeros((self.N[0]-2))
        ...
        self.A0BC=cp.repeat(cp.repeat(cp.array([[[0]],[[0]]]),self.N[2],axis=-1),self.N[1],axis=-2).astype('complex128') # Is this right?
        self.ck=cp.ones((self.N[0],self.N[1],self.N[2]),dtype=cp.complex128); self.ck[0,:,:]=.5; self.ck[-1,:,:]=0; self.ck=self.ck/self.Re 
        # Matrices used multiple times
        self.nones=(-cp.ones(self.N[0]))**cp.arange(self.N[0])
        [self.Mfe,self.Mfo]=self.Mf()

        # Initialize things for the Helmholtz solver
        self.utemp=cp.zeros((self.N[0],self.N[1],self.N[2]),dtype=cp.complex128)
        self.Ne=int((self.N[0]-1)/2+1)
        self.No=self.N[0]-self.Ne
        self.Me1=cp.ones((1,self.Ne),dtype=cp.complex128)
        self.Mo1=cp.ones((1,self.No),dtype=cp.complex128)
        self.Ie=cp.eye(self.Ne-1,dtype=cp.complex128)
        self.Io=cp.eye(self.No-1,dtype=cp.complex128)
        ...
        if store:
            # pressure calcs
            lam=self.k2
            self.MeInvP=cp.zeros((self.Ne,self.Ne,self.N[1],self.N[2]),dtype=cp.complex128)
            self.MoInvP=cp.zeros((self.No,self.No,self.N[1],self.N[2]),dtype=cp.complex128)
            for i in range(self.N[1]):
                for j in range(self.N[2]):
                    Me=cp.concatenate((self.Me1,-lam[i,j]*self.Mfe[1:,:]),axis=0)
                    Me[1:,1:]=Me[1:,1:]+self.Ie
                    Mo=cp.concatenate((self.Mo1,-lam[i,j]*self.Mfo[1:,:]),axis=0)
                    Mo[1:,1:]=Mo[1:,1:]+self.Io

                    # compute inverse
                    self.MeInvP[:,:,i,j]=cp.linalg.inv(Me)
                    self.MoInvP[:,:,i,j]=cp.linalg.inv(Mo)

            # velocity calcs
            lam=self.lam*self.Re
            self.MeInvu=cp.zeros((self.Ne,self.Ne,self.N[1],self.N[2]),dtype=cp.complex128)
            self.MoInvu=cp.zeros((self.No,self.No,self.N[1],self.N[2]),dtype=cp.complex128)
            for i in range(self.N[1]):
                for j in range(self.N[2]):
                    Me=cp.concatenate((self.Me1,-lam[i,j]*self.Mfe[1:,:]),axis=0)
                    Me[1:,1:]=Me[1:,1:]+self.Ie
                    ...
                    # compute inverse
                    self.MeInvu[:,:,i,j]=cp.linalg.inv(Me)
                    self.MoInvu[:,:,i,j]=cp.linalg.inv(Mo)
    ...
    def Helmholtz_Loop_par_pass(N1,N2,beta,Mfe,Mfo,fe,fo,g,u,Me1,Mo1,Ie,Io):
        for i in range(N1):
            for j in range(N2):  
                ...
                Me=cp.concatenate((Me1,-beta[i,j]*Mfe),axis=0)
                Me[1:,1:]=Me[1:,1:]+Ie
                Mo=cp.concatenate((Mo1,-beta[i,j]*Mfo),axis=0)
                Mo[1:,1:]=Mo[1:,1:]+Io
                ...
                Fe=cp.concatenate((ge,Mfe@fe[:,i,j]))
                u[::2,i,j]=cp.linalg.solve(Me,Fe)
                Fo=cp.concatenate((go,Mfo@fo[:,i,j]))
                u[1::2,i,j]=cp.linalg.solve(Mo,Fo)
    ...
    def Mf(self,):
        ...
        No=self.N[0]-Ne
        Me=cp.zeros((Ne,Ne),dtype=cp.complex128)
        Mo=cp.zeros((No,No),dtype=cp.complex128)
    ...
    def compl_store(self,BC,lam):
        ...
        P1c=cp.zeros((self.N[0],self.N[1],self.N[2]),dtype=cp.complex128)
        v1c=cp.zeros((self.N[0],self.N[1],self.N[2]),dtype=cp.complex128)
        P2c=cp.zeros((self.N[0],self.N[1],self.N[2]),dtype=cp.complex128)
        v2c=cp.zeros((self.N[0],self.N[1],self.N[2]),dtype=cp.complex128)

        # Compute the pressure homogenous solutions
        PBC=cp.repeat(cp.repeat(cp.array([[[1]],[[0]]]),self.N[2],axis=-1),self.N[1],axis=-2).astype('complex128') # Is this right?
        P1c=cp.copy(self.Helmholtz_Loop(self.k2,cp.zeros((self.N[0],self.N[1],self.N[2]),dtype=cp.complex128),PBC,'P'+self.style))
        PBC=cp.repeat(cp.repeat(cp.array([[[0]],[[1]]]),self.N[2],axis=-1),self.N[1],axis=-2).astype('complex128') # Is this right?
        P2c=cp.copy(self.Helmholtz_Loop(self.k2,cp.zeros((self.N[0],self.N[1],self.N[2]),dtype=cp.complex128),PBC,'P'+self.style))

        # Compute RHS for helmholtz solver
        fv1=self.dy(P1c[:,:,:,cp.newaxis,cp.newaxis],0,0,0,0).squeeze()
        fv2=self.dy(P2c[:,:,:,cp.newaxis,cp.newaxis],0,0,0,0).squeeze()
        ...
        v1c=cp.copy(self.Helmholtz_Loop(lam*self.Re,fv1*self.Re,BC,'u'+self.style))
        v2c=cp.copy(self.Helmholtz_Loop(lam*self.Re,fv2*self.Re,BC,'u'+self.style))
        ...
        #Compute c1 and c2
        Dv1=self.dy(v1c[:,:,:,cp.newaxis,cp.newaxis],0,0,0,0).squeeze()
        Dv2=self.dy(v2c[:,:,:,cp.newaxis,cp.newaxis],0,0,0,0).squeeze()
        #Make the matrix
        Dv1t=cp.sum(Dv1,axis=0)
        Dv1b=cp.sum(Dv1*self.nones[:,cp.newaxis,cp.newaxis],axis=0)
        Dv2t=cp.sum(Dv2,axis=0)
        Dv2b=cp.sum(Dv2*self.nones[:,cp.newaxis,cp.newaxis],axis=0)
        Dv=cp.asarray([[Dv1t,Dv2t],[Dv1b,Dv2b]]).squeeze()
    ...
    def Pv(self,S,Sp,BC,lam,inpy=0,outy=0,inpxz=0,outxz=0): # Solves P''-k2P=Sp 1/Rev''-lam v- P'=S
        ...
        if inpy==1:
            S=self.cheb(S[:,:,:,cp.newaxis,cp.newaxis]).squeeze()
            Sp=self.cheb(Sp[:,:,:,cp.newaxis,cp.newaxis]).squeeze()
        if inpxz==1:
            S=self.fft(S[:,:,:,cp.newaxis,cp.newaxis]).squeeze()
            Sp=self.fft(Sp[:,:,:,cp.newaxis,cp.newaxis]).squeeze()

        #######################################################################
        # Compute the particular solutions for P and u2 and the cs from the complimentary sol
        #######################################################################       
        Pp=cp.zeros((self.N[0],self.N[1],self.N[2]),dtype=cp.complex128)
        vp=cp.zeros((self.N[0],self.N[1],self.N[2]),dtype=cp.complex128)
        c=cp.zeros((2,self.N[1],self.N[2]),dtype=cp.complex128)
        ctemp=cp.zeros((2,self.N[1],self.N[2]),dtype=cp.complex128)
        # Get the particular solution for the pressure
        PBC=cp.repeat(cp.repeat(cp.array([[[0]],[[0]]]),self.N[2],axis=-1),self.N[1],axis=-2).astype('complex128') # Is this right?
        Pp=cp.copy(self.Helmholtz_Loop(self.k2,Sp,PBC,'P'+self.style))

        # Compute the RHS used in the Helmholtz solver
        fv=S+self.dy(Pp[:,:,:,cp.newaxis,cp.newaxis],0,0,0,0).squeeze() # Solving this outside the loop is much faster
        # Get the particular solution for the velocity and solver for the pressure BC
        vp=cp.copy(self.Helmholtz_Loop(lam*self.Re,fv*self.Re,BC,'u'+self.style))  
        ...
        Dvp=self.dy(vp[:,:,:,cp.newaxis,cp.newaxis],0,0,0,0).squeeze()
        Dvpt=cp.sum(Dvp,axis=0)
        Dvpb=cp.sum(Dvp*self.nones[:,cp.newaxis,cp.newaxis],axis=0)
        b=cp.asarray([[Dvpt],[Dvpb]]).squeeze()

        den = self.Dv[0,0,:,:]*self.Dv[1,1,:,:] - self.Dv[0,1,:,:]*self.Dv[1,0,:,:]

        c0 = self.Dv[1,1,:,:]*-b[0,:,:] - self.Dv[0,1,:,:]*-b[1,:,:]
        c1 = -self.Dv[1,0,:,:]*-b[0,:,:] + self.Dv[0,0,:,:]*-b[1,:,:]
        #c=cp.asarray([self.Dv[1,1,:,:]*-b[0,:,:]-self.Dv[0,1,:,:]*-b[1,:,:] ,-self.Dv[1,0,:,:]*-b[0,:,:]+self.Dv[0,0,:,:]*-b[1,:,:]])/(self.Dv[0,0,:,:]*self.Dv[1,1,:,:]-self.Dv[0,1,:,:]*self.Dv[1,0,:,:])
        c = cp.stack((c0, c1), axis=0) / den
        c[:,0,0]=cp.zeros(2)
        ...
        if outy==1:
            P=self.icheb(P[:,:,:,cp.newaxis,cp.newaxis]).squeeze()
            v=self.icheb(v[:,:,:,cp.newaxis,cp.newaxis]).squeeze()
        if outxz==1:
            P=self.ifft(P[:,:,:,cp.newaxis,cp.newaxis]).squeeze()
            v=self.ifft(v[:,:,:,cp.newaxis,cp.newaxis]).squeeze()
    ...
    def uw(self,S,P,v,lam,inpy=0,outy=0,inpxz=0,outxz=0):
        ...
        if inpy==1:
            S=self.cheb(S[:,:,:,cp.newaxis,cp.newaxis]).squeeze()
            P=self.cheb(P[:,:,:,cp.newaxis,cp.newaxis]).squeeze()
            v=self.cheb(v[:,:,:,cp.newaxis,cp.newaxis]).squeeze()
        if inpxz==1:
            S=self.fft(S[:,:,:,cp.newaxis,cp.newaxis]).squeeze()
            P=self.fft(P[:,:,:,cp.newaxis,cp.newaxis]).squeeze()
            v=self.fft(v[:,:,:,cp.newaxis,cp.newaxis]).squeeze()
        ...
        # Solve for u and w
        BC=cp.repeat(cp.repeat(cp.array([[[0]],[[0]]]),self.N[2],axis=-1),self.N[1],axis=-2).astype('complex128') # Is this right?
        w=cp.copy(self.Helmholtz_Loop(lam*self.Re,fw*self.Re,BC,'u'+self.style))
        BC[:,0,0]=cp.array([1,-1])
        u=cp.copy(self.Helmholtz_Loop(lam*self.Re,fu*self.Re,BC,'u'+self.style))
        u=cp.concatenate((u[:,:,:,cp.newaxis],v[:,:,:,cp.newaxis],w[:,:,:,cp.newaxis]),axis=-1)
        ...
        u[-1,1:,0,0]=0
        u[-1,0,1:,2]=0
        u[-1,1:,0,0]=cp.real(u[-1,1:,0,0])
        u[-1,0,1:,2]=cp.real(u[-1,0,1:,2])
        u[:,0,0,:]=cp.real(u[:,0,0,:])

        if outy==1:
            u=self.icheb(u[:,:,:,:,cp.newaxis]).squeeze()
        if outxz==1:
            u=self.ifft(u[:,:,:,:,cp.newaxis]).squeeze()
    ...
    def tau_cor(self,P0,v0,P1,v1,S,lam):
        ...
        sig0=cp.concatenate((sig00,sig01),axis=0)

        # Compute sigma for The A1 problem
        sig10=2*(N2-1)*(self.Re*S[-2:-1,:,:]+beta*v1[-2:-1,:,:]+2*N2*P1[-1:,:,:]*self.Re)
        sig11=2*N2*(self.Re*S[-1:,:,:]+beta*v1[-1:,:,:])
        sig1=cp.concatenate((sig10,sig11),axis=0)

        # Compute the correction
        sigt=sig1/(1-sig0)
        idx=int(cp.ceil(self.N[0]/2))
        sigt=cp.repeat(sigt,idx,axis=0)
        sigp=cp.zeros(sigt.shape,dtype=cp.complex128)
        sigp[0::2,:,:]=sigt[:idx,:,:]
        sigp[1::2,:,:]=sigt[idx:,:,:]
        sigv=cp.roll(sigp,1,axis=0)
        sigp=sigp[:-1,:,:]
        sigv=sigv[:-1,:,:]     
    ...
    def reward(self,u,p):
        ...
        tau=cp.real(self.dy(u[:,:,:,0:1,cp.newaxis],0,1,0,1).squeeze())
        taut=tau[-1,:,:]
        taub=tau[0,:,:]

        # Compute the drag on the wall
        dA=self.L[1]/self.N[1]*self.L[2]/self.N[2]
        dragt=cp.sum(taut-1)*dA
        dragb=cp.sum(taub-1)*dA
    ...
    def Step(self,u,P,BC,tau=True,scheme='Multi',fixedBC=False,inpy=0,outy=0,inpxz=0,outxz=0):
        ...
        if scheme=='Multi':
            ...
            if fixedBC==False:
                ...
                self.P0,self.v0=self.Pv(cp.zeros((self.N[0],self.N[1],self.N[2]),dtype=cp.complex128),self.ck,self.A0BC,self.lam,0,0,0,0) # depends on the comp sol
            ...
            for i in range(3):
                ...
                if tau==True:
                    # Compute the A0 problem
                    P0,v0=self.Pv(cp.zeros((self.N[0],self.N[1],self.N[2]),dtype=cp.complex128),self.ck,self.A0BC,self.lam_init[i],0,0,0,0)
                    # Use the A0 and A1 problems to solve for P and v
                ...
                utemp=cp.concatenate((utemp,unew[:,:,:,:,cp.newaxis]),axis=-1)
                Ptemp=P[:,:,:,cp.newaxis,cp.newaxis]
